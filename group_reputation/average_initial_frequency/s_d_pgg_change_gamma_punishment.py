...
def game_one_round(stra_l, gamma_l, ind_pos, pos_ind, gamma_1, gamma_2):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    stra_l_old = stra_l[:]
...
def run_game(run_time, gamma_1, gamma_2, ind_pos, pos_ind):
    ...
    for step in range(run_time):
        stra_l, gamma_l = game_one_round(stra_l, gamma_l, ind_pos, pos_ind, gamma_1, gamma_2)
    return stra_l, gamma_l


def evaluation(eval_time, gamma_1, gamma_2, ind_pos, pos_ind, stra_l, gamma_l):
    ind_n = len(ind_pos)
...
if __name__ == '__main__':
    ...
    run_time = 1000; gamma_1 = 0.5; gamma_2 = 0.1; eval_time = 200
    init_time = 10
    r_stra_frac = np.array([0, 0, 0])
    for i in range(init_time):
        stra_l, gamma_l = run_game(run_time, gamma_1, gamma_2, ind_pos, pos_ind)
        stra_frac = evaluation(eval_time, gamma_1, gamma_2, ind_pos, pos_ind, stra_l, gamma_l)
