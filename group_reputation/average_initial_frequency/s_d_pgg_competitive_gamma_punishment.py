...
if __name__ == '__main__':
    ...
    for gamma in np.arange(0.1, 1.3, 0.1):
        ave_gamma = round(gamma, 2)
        print(ave_gamma)
        for i in range(init_time):
            stra_l, gamma_l = run_game(run_time, ave_gamma, ind_pos, pos_ind)
            stra_frac = evaluation(eval_time, ave_gamma, ind_pos, pos_ind, stra_l, gamma_l)
            r_stra_frac = i / (i + 1) * r_stra_frac + 1 / (i + 1) * stra_frac
        result[gamma] = r_stra_frac
    result = pd.DataFrame(result).T
    result.to_csv('./results/s_d_pgg_competitive_gamma_punishment.csv')
    print(result)