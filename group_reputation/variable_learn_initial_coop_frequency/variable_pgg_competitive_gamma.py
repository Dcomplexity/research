...
def game_one_round(a_l, gamma_l, ind_pos, pos_ind, ave_gamma, update_ind_num):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    a_l_old = a_l[:]
...
def run_game(run_time, ave_gamma, ind_pos, pos_ind, update_ind_num):
    ...
    for step in range(run_time):
        a_l, gamma_l = game_one_round(a_l, gamma_l, ind_pos, pos_ind, ave_gamma, update_ind_num)
    return a_l, gamma_l


def evaluation(eval_time, ave_gamma, ind_pos, pos_ind, a_l, gamma_l, update_ind_num):
    ind_n = len(ind_pos)
    a_frac = np.array([0, 0])
    ...
    for update_ind_num_r in update_ind_num_l_r:
        for ave_gamma_r in ave_gamma_l_r:
            ...
            for i in range(init_time):
                a_l_r, gamma_l_r = run_game(run_time, ave_gamma_r, ind_pos_r, pos_ind_r, update_ind_num_r)
                a_frac_r = evaluation(eval_time, ave_gamma_r, ind_pos_r, pos_ind_r, a_l_r, gamma_l_r, update_ind_num_r)
