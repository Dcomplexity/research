...
def initialize_strategy(t_n):
    ...
    init_stra = np.zeros(t_n, dtype=int)
    for i in range(t_n):
        init_stra[i] = np.random.choice([0, 1, 2], p = [(i+1)/t_n, (1.0-(i+1)/t_n)/2.0, (1.0-(i+1)/t_n)/2.0])
    return init_stra
...
if __name__ == '__main__':
    ...
    result_file_name = './results_old/hete_pgg_competitive_gamma_punishment_10.csv'
