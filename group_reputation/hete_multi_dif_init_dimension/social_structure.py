...
def build_hete_pos_ind(mul_dimen, pos_ind, g_l, g_s):
    pos_ind_hete = []
    for _ in range(mul_dimen):
        pos_ind_hete.append(new_hete_structure(pos_ind, g_s, g_l))
    return pos_ind_hete


def build_hete_ind_pos(pos_ind_mul, t_n):
    ind_pos_hete = [[0 for i in range(t_n)] for j in range(len(pos_ind_mul))]
    for i in range(len(pos_ind_mul)):
        for j in range(len(pos_ind_mul[i])):
            for k in pos_ind_mul[i][j]:
                ind_pos_hete[i][k] = j
...
if __name__ == '__main__':
    ...
    pos_ind_hete = build_hete_pos_ind(mul_dimen, pos_ind, group_length, group_size)
    ind_pos_hete = build_hete_ind_pos(pos_ind_hete, total_number)
    print(pos_ind_hete)
