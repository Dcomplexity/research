...
def run_game_frac_time(run_time, gamma, ind_pos, pos_ind):
    ...
    for step in range(run_time):
        stra_l, p_l = game_one_round(stra_l, gamma, ind_pos, pos_ind)
        stra_history.append(np.copy(stra_l[:]))
        payoff_history.append(np.copy(p_l[:]))
    stra_l, p_l = game_one_round(stra_l, gamma, ind_pos, pos_ind)
    payoff_history.append(np.copy(p_l[:]))
    return stra_history, payoff_history
...
if __name__ == '__main__':
    ...
    for gamma_r in gamma_l:
        print(gamma_r)
        stra_history_r, payoff_history_r = run_game_frac_time(run_time, gamma_r, ind_pos_r, pos_ind_r)
    ...
    result_stra_pd.to_csv('./results_old/ft_pgg_original_punishment_10_stra.csv')
    result_payoff_pd.to_csv('./results_old/ft_pgg_original_punishment_10_payoff.csv')
