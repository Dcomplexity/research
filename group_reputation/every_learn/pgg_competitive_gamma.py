...
def game_one_round(a_l, g_gamma_l, ind_pos, pos_ind, g_s, w, ave_gamma):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    a_l_old = np.copy(a_l)
    ind_a_l = a_l.flatten()
    ...
    for pos in range(pos_n):
        ...
        t1 = (1 / 2 + 2 / (2 * delta_pi) * (oppon_p - ind_p))
        t2 = random.random()
        if t2 < t1:
    ...
    for pos in range(pos_n):
        g_gamma_l[pos] = ave_gamma * pos_n * (g_a_frac[pos] + 0.001) / (total_a_frac + 0.001 * pos_n)

    return ind_a_l.reshape((pos_n, int(ind_n / pos_n))), g_gamma_l


def run_game(f_0, init_time, run_time, ave_gamma, ind_pos, pos_ind, g_s, w):
    f_history = []
    pos_n = len(pos_ind)
    for round in range(init_time):
        ...
        for step in range(run_time):
            ...
            a_l, g_gamma_l = game_one_round(a_l, g_gamma_l, ind_pos, pos_ind, g_s, w, ave_gamma)
        if round == 0:
            f_history.append(a_l.mean(axis=1))
        else:
...
if __name__ == '__main__':
    g_s = 20; g_b = 2; g_l = 4; w = 1.0; run_time = 10000; init_time = 100
    g_n = g_b ** (g_l - 1); c = 1.0
    ind_pos, pos_ind = build_structure(g_s, g_b, g_l)
    gamma_l = np.round(np.arange(0.1, 1.3, 0.1), 2)
    step_l = np.arange(run_time + 1)
    gamma_frac_history = []
    for ave_gamma in gamma_l:
        print(ave_gamma)
        f_0 = np.random.random(g_n)
        history_sim_r = run_game(f_0, init_time, run_time, ave_gamma, ind_pos, pos_ind, g_s, w)
    ...
    gamma_frac_history_pd.to_csv('./results_old/pgg_competitive_gamma.csv')
