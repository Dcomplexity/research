...
if __name__ == '__main__':
    g_n = 30; g_s = 5; w = 1.0; c = 1.0; mu = 0.01; run_time = 1000; init_time = 100
    init_type = 'homo'
    ind_pos, pos_ind = build_structure(g_s, g_n)
    gamma_l = np.round(np.arange(0.1, 1.51, 0.05), 2)
    step_l = np.arange(run_time + 1)
    for r_value in [2, 2.5, 3, 5]:
        print(r_value)
        gamma_frac_history_simulation = []
        gamma_frac_history_dynamics = []
        adj_link, edge = generate_price(g_n, 3, r_value)
        for gamma in gamma_l:
            print(gamma)
            f_0 = [0.5 for _ in range(g_n)]
            group_c_frac_history_sim_r = run_game(f_0, init_time, run_time, gamma, ind_pos, pos_ind, g_s, w, mu, adj_link)
            group_c_frac_history_dy_r = dynamic_process(g_n, g_s, c, gamma, mu, run_time, init_type, adj_link)
        ...
        csv_file_name_simulation = './results/pgg_original_simulation_unique_price_network_%.1f.csv' % r_value
        gamma_frac_history_simulation_pd.to_csv(csv_file_name_simulation)
        csv_file_name_dynamics = './results/pgg_original_dynamics_unique_price_network_%.1f.csv' % r_value
