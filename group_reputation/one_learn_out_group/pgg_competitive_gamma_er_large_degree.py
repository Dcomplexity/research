...
def generate_er(degree, popu_num):
    # g_network = nx.erdos_renyi_graph(popu_num, p=degree/(popu_num-1))
    g_network = nx.gnm_random_graph(popu_num, popu_num * 4)
    gen_num = 0
...
def run_game(f_0, init_time, run_time, ave_gamma, ind_pos, pos_ind, g_s, w, mu):
    ...
    for round in range(init_time):
        adj_link, edge = generate_er(4, pos_n)
...
if __name__ == '__main__':
    g_s = 8; g_b = 2; g_l = 7; w = 1.0; run_time = 1000; init_time = 100
    ...
    gamma_frac_history_pd.to_csv('./results_old/pgg_competitive_gamma_er_large_degree.csv')
