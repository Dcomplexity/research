...
def game_one_round(stra_l, gamma, ind_pos, pos_ind, pun):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    stra_l_old = stra_l[:]
...
def run_game(run_time, gamma, ind_pos, pos_ind, pun):
    ind_n = len(ind_pos)
    stra_l = initialize_strategy(ind_n)
    for step in range(run_time):
        stra_l = game_one_round(stra_l, gamma, ind_pos, pos_ind, pun)
    return stra_l

def evaluation(eval_time, gamma, ind_pos, pos_ind, stra_l, pun):
    ind_n = len(ind_pos)
...
if __name__ == '__main__':
    ...
    for pun_r in pun_list_r:
        print(pun_r)
        for gamma_r in gamma_list_r:
            print(gamma_r)
            for i in range(init_time):
                stra_l_r = run_game(run_time, gamma_r, ind_pos_r, pos_ind_r, pun_r)
                stra_frac_r = evaluation(eval_time, gamma_r, ind_pos_r, pos_ind_r, stra_l_r, pun_r)
    ...
    result_pd.to_csv('./results_old/s_d_pgg_original_punishment_10.csv')
    print(result_pd)