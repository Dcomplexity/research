...
def game_one_round(stra_l, gamma_l, ind_pos, pos_ind, ave_gamma, pun):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    stra_l_old = stra_l[:]
        ...
        if stra_count[0] > 0:
            for i in range(g_inds_n):
                if stra_l[g_inds[i]] == 2:
                    p_l[g_inds[i]] -= pun
                if stra_l[g_inds[i]] == 0:
                    p_l[g_inds[i]] -= pun * stra_count[2] / stra_count[0]
...
def run_game(run_time, ave_gamma, ind_pos, pos_ind, pun):
    ind_n = len(ind_pos)
    pos_n = len(pos_ind)
    stra_l = initialize_strategy(ind_n)
    gamma_l = initialize_gamma(pos_n, ave_gamma)
    for step in range(run_time):
        stra_l, gamma_l = game_one_round(stra_l, gamma_l, ind_pos, pos_ind, ave_gamma, pun)
    return stra_l, gamma_l


def evaluation(eval_time, ave_gamma, ind_pos, pos_ind, stra_l, gamma_l, pun):
    ind_n = len(ind_pos)
...
if __name__ == '__main__':
    ...
    result = []
    gamma_list_r = np.round(np.arange(0.1, 1.3, 0.1), 2)
    pun_list_r = np.round(np.arange(0.1, 2.1, 0.1), 2)
    for pun_r in pun_list_r:
        print(pun_r)
        for ave_gamma_r in gamma_list_r:
            print(ave_gamma_r)
            for i in range(init_time):
                stra_l_r, gamma_l_r = run_game(run_time, ave_gamma_r, ind_pos_r, pos_ind_r, pun_r)
                stra_frac_r = evaluation(eval_time, ave_gamma_r, ind_pos_r, pos_ind_r, stra_l_r, gamma_l_r, pun_r)
