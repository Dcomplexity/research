...
class SocialStructure():
    def __init__(self, g_s, g_b, g_l, total_num):
        self.g_s = g_s
        self.g_b = g_b
        self.g_l = g_l
...
def generate_distance_prob(g_l, g_s, reg_param):
    distance_dist = np.zeros(g_l)
    # If there is only on individual in a group, individual should not interact with himself.
    if g_s == 1:
