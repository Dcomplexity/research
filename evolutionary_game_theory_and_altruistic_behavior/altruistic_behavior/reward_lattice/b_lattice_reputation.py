...
def evolution_one_step(popu, total_num, edges, b):
    ...
    for i in range(total_num):
        ...
        for j in play_agent_l:
            r_i, r_j = play_b_game(a_l[i], a_l[j], b)
    ...
    for i in range(total_num):
        if popu[i].get_strategy() == 0 or popu[i].get_strategy() == 2:
            a_new_l[i] = 0
        else:
            a_new_l[i] = 1
    for i in range(total_num):
        if a_l[i] == 1:
            ...
            for j in poten_agent:
                if ind_rep[i] > 3 and ind_rep[j] > 3:
                    r_i, r_j = play_b_game(a_new_l[i], a_new_l[j], b)
                    popu[i].add_payoffs(r_i)
                    popu[j].add_payoffs(r_j)
...
if __name__ == '__main__':
    simulation_name = "b_lattice_reputation"
    ...
    init_num = 2
    b_r_l = []
    for i in np.arange(1.0, 1.51, 0.1):
