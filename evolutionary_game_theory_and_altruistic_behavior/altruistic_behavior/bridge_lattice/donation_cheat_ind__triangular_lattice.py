...
def evolution_one_step(popu, total_num, edges, b, cost):
    ...
    a_l_new = [0 for _ in range(total_num)]
    for i in range(total_num):
        if popu[i].get_strategy() == 0 or popu[i].get_strategy() == 3:
            a_l_new[i] = 0
        else:
            a_l_new[i] = 1
    for i in range(total_num):
            ...
            if len(poten_agent) > 2:
                ...
                for co_pair in poten_combination:
                    for _ in range(10):
                        ...
                        co_i = co_pair[0]
                        co_j = co_pair[1]
                        r_i, r_j = play_donation_game(a_l_new[co_i], a_l_new[co_j], b)
...
if __name__ == '__main__':
    ...
    init_num = 2
    cost_r_l = []
    for i in np.arange(0.4, 0.41, 0.2):
        cost_r_l.append(round(i, 2))
    b_r_l = []
    for i in np.arange(1.6, 2.01, 0.5):
