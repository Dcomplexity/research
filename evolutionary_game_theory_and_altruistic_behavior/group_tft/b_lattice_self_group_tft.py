...
class Agent:
     ...
     def update_env(self, co_frac, method='set_frac'):
         if method == "set_frac":
             self.env = co_frac
         else:
             self.env = self.env + 0.01 * (co_frac - self.env)
...
def evolution_one_step(popu, total_num, edges, b):
    ...
    for i in range(total_num):
        if popu[i].get_strategy() == 0:
            ...
            a_l[i] = 1
        elif popu[i].get_strategy() == 2:
            if random.random() < popu[i].get_env():
                a_l[i] = 1
            else:
                a_l[i] = 0
    ...
    for i in range(total_num):
        co_frac_i = 0
        neighborhood = popu[i].get_link()
        # neighborhood.append(i)
        for j in neighborhood:
            if a_l[j] == 1:
                co_frac_i += 1
        co_frac_i = co_frac_i / len(neighborhood)
        popu[i].update_env(co_frac_i, method='set_frac')
    print(np.mean(a_l))
    return popu
