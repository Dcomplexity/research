...
def get_evol_data():
    ...
    Simulates the dynamics of the stochastic game, and of the two corresponding repeated games
    :return:
    ...
    freqs = np.zeros((1, 2 ** 4)).flatten()
    freq1 = np.zeros((1, 2 ** 4)).flatten()
    freq2 = np.zeros((1, 2 ** 4)).flatten()
    # Define the transitions of the three scenarios
    # In each q, there three cases, 2 C (CC), 2 D (CD or DC), 2D (DD)
    qs = np.array([1, 0, 0]) # the scenario that transition between state 1 and state 2
    q1 = np.array([1, 1, 1]) # only in the state 1
    q2 = np.array([0, 0, 0]) # only in the state 2
    # Vector with all possible one-shot payoffs
    pi_round = np.array([b1 - c, -c, b1, 0, b2 - c, -c, b2, 0])

    for i in range(n_it): # run the evolution process with n_it initializations
        print(i)
        (coop, freq) = evol_proc(qs, pi_round, beta, n_gen)
        # get the average result of n_it initializations
