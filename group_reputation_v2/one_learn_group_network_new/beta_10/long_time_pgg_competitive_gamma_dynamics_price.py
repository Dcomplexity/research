        ...
        if pos_i != pos_j:
            ...
            for c_j in range(n + 1):
                t_plus_p_j += ((n - c_num) / n) * (c_j / n) * c_group_dist[c_j] * neigh[pos_j] * (1 / neigh_num) \
                              * (1 / (1 + math.e ** (10.0 * (payoff[pos_i][c_num][0] - payoff[pos_j][c_j][1]))))
            t_plus_p += t_plus_p_j
        else:
            t_plus_p_i = ((n - c_num) / n) * (c_num / n) * (1 / neigh_num) \
                          * (1 / (1 + math.e ** (10.0 * (payoff[pos_i][c_num][0] - payoff[pos_i][c_num][1]))))
            t_plus_p += t_plus_p_i
        ...
        if pos_i != pos_j:
            ...
            for c_j in range(n + 1):
                t_minus_p_j += (c_num / n) * ((n - c_j) / n) * c_group_dist[c_j] * neigh[pos_j] * (1 / neigh_num) \
                               * (1 / (1 + math.e ** (10.0 * (payoff[pos_i][c_num][1] - payoff[pos_j][c_j][0]))))
            t_minus_p += t_minus_p_j
        else:
            t_minus_p_i = (c_num / n) * ((n - c_num) / n) * (1 / neigh_num) \
                          * (1 / (1 + math.e ** (10.0 * (payoff[pos_i][c_num][1] - payoff[pos_i][c_num][0]))))
...
if __name__ == '__main__':
    ...
    for r_value in [2, 2.2, 2.5, 3, 5, 7, 10]:
        ...
        csv_file_name = './results_beta_10/long_time_pgg_competitive_gamma_dynamics_price_%.1f.csv' % r_value
