...
def initialize_population():
    a_l = [0, 1, 2]  # 0 for defect, 1 for cooperate and 2 for punish
    a_num = len(a_l)
...
def rl_in_pgg(popu, total_num, edges, r, eval=False):
    ...
    for i in range(total_num):
        ...
        cd_neigh = []
        for a_neigh_item in a_neigh:
            if a_neigh_item == 0:
                cd_neigh.append(0)
            else:
                cd_neigh.append(1)
        r_neigh_l = play_pgg_game_punishment(cd_neigh, enhancement_factor, a_neigh, 1, 0.5)
...
def run(r, run_time):
    ...
    for _ in range(run_time):
        popu = rl_in_pgg(popu, total_num, edges, r, eval=False)
    return popu, network, total_num, edges
...
def evaluation(popu, edges, r, sample_time):
    ...
    for _ in range(sample_time):
        ...
        sample_action_frac.append(a_l_dist)
    return np.mean(sample_action_frac, axis=0)


if __name__ == '__main__':
    start_time = datetime.datetime.now()
    simulation_name = "pgg_lattice_rl_punishment"
    ...
    idx = pd.Index(r_r_l)
    idx.set_names('r')
    result_pd = pd.DataFrame(result_l, index=idx, columns=['def_frac', 'co_frac', 'pun_frac'])
