...
def evolution_one_step(popu, total_num, edges, r, discount):
    for i in range(total_num):
    ...
    for i in range(total_num):
        ...
        p_group_l = play_pgg_game_discount_cost(a_group, enhancement_factor, discount)
...
def run(r, discount, run_time):
    ...
    for _ in range(run_time):
        popu = evolution_one_step(popu, total_num, edges, r, discount)
    return popu, network, total_num, edges
...
if __name__ == '__main__':
    discount_r = 0.5
    simulation_name = 'pgg_well_mixed_imitation_discount_cost_%s' % discount_r
